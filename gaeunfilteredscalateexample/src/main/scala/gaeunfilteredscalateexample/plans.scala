...
class SayHelloPlan extends ScalateConfigurator with unfiltered.filter.Plan with com.weiglewilczek.slf4s.Logging {
  def intent = {
    ...
    case req @ GET(Path(Seg("hello" :: name :: Nil))) => Ok ~> Scalate(req, "templates/hello.ssp", ("name", name))
    case _ => Pass
  }
}
...
class SayGoodbyePlan extends ScalateConfigurator with unfiltered.filter.Plan with com.weiglewilczek.slf4s.Logging {
  def intent = {
    case req @ Path(Seg("goodbye" :: Nil)) => req match {
      case GET(_) => Ok ~> Scalate(req, "templates/goodbye.ssp")
      case POST(_) => {
        val Params(form) = req
            logger.info("Received post with name: " + form("name"))
            form("name") match {
              case Nil => Redirect("/goodbye")
              case _ => Ok ~> Scalate(req, "templates/goodbye.ssp", ("name", form("name")(0)))
        }
      }
    }
    case req @ GET(Path(Seg("goodbye" :: name :: Nil))) => Ok ~> Scalate(req, "templates/goodbye.ssp", ("name", name))
    case _ => Pass
  }
}
